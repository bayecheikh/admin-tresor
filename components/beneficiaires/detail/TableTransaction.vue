...
<div>
  ...
  <v-data-table
        :headers="headers"
        :items="detailBeneficiaire && detailBeneficiaire.transactions"
        :single-select="singleSelect"
        item-key="id"
        class="flat pt-4"
        :loading="detailBeneficiaire.transactions?false:true"
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listtransactions: 'transactions/listtransactionsByUsager',
      headers: 'transactions/headertransactions2',
    ...
    methods: {
      async getList(){
        ...
        .then(async (response) => {
          console.log('Reponse données transactions reçu by usager +++++ : ', response.data)
          this.$store.dispatch('transactions/getListByUsager',response.data)
        }).catch((error) => {
      ...
      async deleteItem () {
        ...
        this.$essApi.$delete('/transactions/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('transactions/deletetransactionByUsager',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          this.$router.push('/transactions/detail/'+transaction.name);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let transaction = this.selected.map(function(value){ return value})[0]
          this.$store.commit('transactions/initdetail',transaction)
          this.$router.push('/transactions/modifier/'+transaction.name);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

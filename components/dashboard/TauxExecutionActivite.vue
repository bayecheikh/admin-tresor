    ...
    <div class="custom-container mt-5">
      ...
      <v-row class="d-flex justify-content-between">
        <v-col md="12" sm="12" lg="12" class="text-sm-center bg-grey-2 my-5">
          <h2 class="text-grey">Taux d'execution financière par activité (en %)</h2>
        </v-col>
        <v-col md="12" sm="12" lg="12" class="pl-8">
          <v-row class="d-flex justify-content-between">
            <v-col md="4" sm="12" lg="4" class="" v-for="activite in modelAtteintActivite" :key="activite.libelle_activite">
              <div class="bg-marron-2 mr-3 pl-4 pr-5 pt-5 pb-5 text-sm-center">
                <h4 class="">{{activite.libelle_activite}}</h4>
                <h2 class="text-primary mt-2">{{activite.montant?activite.montant.toLocaleString():0}}%</h2>
              </div>
  ...
  export default {
    ...
    methods:{      
        ...
        .then(async (response) => { 
          ...
          await response.data.reduce(function(res, value) {
            if (!res[value.libelle_activite]) {
              res[value.libelle_activite] = { libelle_activite: value.libelle_activite, montant: 0, taux:0 };
              result.push(res[value.libelle_activite])
            }
            res[value.libelle_activite].montant = res[value.libelle_activite].montant + parseInt(value.montant);
            return res;
          }, {});
          
          this.modelAtteintActivite = result
          this.modelAtteintActivite = this.modelAttenduActivite.map((attendu) => {
            const taux = result.filter((atteint) => attendu.libelle_activite === atteint.libelle_activite);

            return {libelle_activite:taux[0]?.libelle_activite,montant:taux[0]?.montant/attendu.montant*100}
          });
          console.log('taux +++++++++ ',modelAtteintActivite)
          let total = 0
          result.map((item)=>{              
      ...
      getFinancementActiviteCible(){
        this.$msasApi.$get('/financement_activites')
        .then(async (response) => { 
          ...
          console.log(result)
          await this.getFinancementActiviteResultat()

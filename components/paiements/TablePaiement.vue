...
<v-data-table
  ...
  items-per-page="5"
  class="flat pt-4"
  :loading="listpaiements.length<0?true:false"
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      listpaiements: 'paiements/listpaiements',
      headers: 'paiements/headerpaiements'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('paiements/getDetail',item);
        this.$router.push('/paiements/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('paiements/getDetail',item);
        this.$router.push('/paiements/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})
        this.$msasApi.$delete('/paiements/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('paiements/deletepaiement',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let paiement = this.selected.map(function(value){ return value})[0]
          this.$store.commit('paiements/initdetail',paiement)
          this.$router.push('/paiements/detail/'+paiement.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let paiement = this.selected.map(function(value){ return value})[0]
          this.$store.commit('paiements/initdetail',paiement)
          this.$router.push('/paiements/modifier/'+paiement.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

<template>
  ...
  <v-data-table
    ...
    :headers="headers"
    :items="tab=='tout'?listresultat_attendus : listresultat_attendus.filter(resultat_attendu => resultat_attendu.status === tab)"

    item-key="id"
    items-per-page="5"

    class="flat pt-4"
    :loading="listresultat_attendus.length?false:true"
...
import RechercheResultatattendu from '@/components/resultat_attendus/RechercheResultatattendu';
  export default {
    components: {
      RechercheResultatattendu
    },
    mounted: function() {
      //this.getList(1)
    },
    computed: mapGetters({
      listresultat_attendus: 'resultat_attendus/listresultat_attendus',
      headers: 'resultat_attendus/headerresultat_attendus',
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      actveDesactiveUser(id) {
        console.log('------------- resultat_attendu active',id)
        this.dialog=false
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})
        this.$msasApi.$get('/active_resultat_attendu/'+id)
        .then(async (response) => {
      ...
      visualiserItem (item) {
        this.$store.dispatch('resultat_attendus/getDetail',item)
        this.$router.push('/resultat_attendus/detail/'+item.id);
      },
      editItem (item) {
        this.$store.dispatch('resultat_attendus/getDetail',item)
        this.$router.push('/resultat_attendus/modifier/'+item.id);
      },
      async deleteItem () {
        this.dialog=false
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})
        this.$msasApi.$delete('/resultat_attendus/'+this.activeItem.id)
        .then(async (response) => {
            this.$store.dispatch('resultat_attendus/deleteUtilisateur',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let resultat_attendu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('resultat_attendus/initdetail',resultat_attendu)
          this.$router.push('/resultat_attendus/detail/'+resultat_attendu.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let resultat_attendu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('resultat_attendus/initdetail',resultat_attendu)
          this.$router.push('/resultat_attendus/modifier/'+resultat_attendu.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
